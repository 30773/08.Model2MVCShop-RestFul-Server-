...
<head>
	...
	<script type="text/javascript">
		...
		$( function() {
			...
			$('.cmt > .form-group > div > button').on('click',function(){
				fncAddComment($(this).parent().parent().parent());
			});
			
			$('input:text[name="cmt"]').on('keydown',function(event){
				if(event.keyCode == '13'){
			...
			$('a.btn-default:contains("목록")').on('click',function(){
				self.location.href = 'listProduct?menu=${param.menu}';
			});

			$('a.btn-success:contains("구매하기")').on('click',function(){
					...
					<div class="col-xs-7">
						...
						<div class="btn-group" role="group">
							<c:if test="${param.menu=='search' && !empty user}">
								<a href="#" class="btn btn-success" role="button">구매하기</a>
							</c:if>
							<a href="#" class="btn btn-default" role="button">목록</a>
						</div>
					</div>
				</div>
